...
class PaymentMethod extends Model
{
    ...
    public function activeList(): array
    {
        return $this->db->fetchAll(
            "SELECT * FROM payment_methods WHERE deleted_at IS NULL ORDER BY method_name ASC"
        );
    }

    public function nameExists(string $name, ?int $excludeId = null): bool
    {
        $sql = "SELECT id FROM payment_methods WHERE method_name = :name AND deleted_at IS NULL";
        $params = ['name' => $name];
        if ($excludeId) {
    ...
    public function listPaginated(array $filters, string $sort, string $dir, int $limit, int $offset): array
    {
        ...
        $sort = $sort === 'created_at' ? 'created_at' : 'method_name';
