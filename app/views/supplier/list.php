<?php
function supplierSortLink(string $col, string $label, string $sort, string $dir): string
{
    $nextDir = ($sort === $col && $dir === 'asc') ? 'desc' : 'asc';
    $url = BASE_URL . '/index.php?module=supplier&sort=' . urlencode($col) . '&dir=' . $nextDir;
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        ...
        <small class="text-muted">Master data supplier</small>
    </div>
    <div class="d-flex gap-2">
        <a href="<?= BASE_URL ?>/index.php?module=master_data" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left"></i> Master Data
        </a>
        <a href="<?= BASE_URL ?>/index.php?module=supplier&action=create" class="btn btn-primary">
    ...
    <div class="card-body">
        <form method="GET" action="<?= BASE_URL ?>/index.php" class="row g-2 align-items-end">
            ...
            <div class="col-md-2 d-flex gap-2">
                ...
                <a href="<?= BASE_URL ?>/index.php?module=supplier" class="btn btn-sm btn-outline-secondary">
                    <i class="bi bi-x-circle"></i>
                </a>
            </div>
            <div class="col-md-2 d-flex gap-2 no-print">
                <a href="<?= BASE_URL ?>/index.php?module=supplier&action=exportCsv&<?= e($baseQuery) ?>" class="btn btn-sm btn-outline-success w-100">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th><?= supplierSortLink('supplier_code', 'Kode', $sort, $dir) ?></th>
                        <th><?= supplierSortLink('supplier_name', 'Nama Supplier', $sort, $dir) ?></th>
                        <th><?= supplierSortLink('contact_person', 'PIC', $sort, $dir) ?></th>
                        <th>Telepon</th>
                        <th>Email</th>
                        <th class="text-center"><?= supplierSortLink('status', 'Status', $sort, $dir) ?></th>
                    ...
                    <?php if (empty($suppliers)): ?>
                        <tr><td colspan="7" class="p-0">
                            <div class="empty-state">
                                ...
                                <div class="empty-title">Belum ada supplier</div>
                                <div class="empty-desc">Tambahkan supplier untuk mulai membuat Purchase Order.</div>
                                <a href="<?= BASE_URL ?>/index.php?module=supplier&action=create" class="btn btn-sm btn-primary">
                                    <i class="bi bi-plus-circle"></i> Tambah Supplier
                                </a>
                            </div>
                        </td></tr>
                            ...
                            <td class="text-center no-print">
                                <div class="dropdown row-actions">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item" href="<?= BASE_URL ?>/index.php?module=supplier&action=edit&id=<?= (int) $s['id'] ?>">
